...
function collectFilterState() {
  return {
    upbjj: document.getElementById("upbjiList")?.value ?? "all",
...
function applyFiltersSort(data, state) {
  ...
  const filtered = data.filter((it) => {
    if (state.upbjj !== "all" && it.upbjj !== state.upbjj) return false;
...
function validateStock(s) {
  const requiredText = ["kode", "judul", "kategori", "upbjj", "lokasiRak", "catatanHTML"];
...
export function initStockTable() {
  ...
  buildOptions(upbjiList, allStocks, "upbjj");
  ...
  stockForm?.addEventListener("submit", (e) => {
    ...
    buildOptions(upbjiList, current, "upbjj");
  ...
  document.body.addEventListener("click", (e) => {
    ...
    if (action === "delete") {
      ...
      buildOptions(upbjiList, next, "upbjj");
